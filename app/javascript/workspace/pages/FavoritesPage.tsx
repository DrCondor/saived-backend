...
function FavoriteCard({ item, onUnfavorite, onClick }: FavoriteCardProps) {
  ...
  return (
    <div
      className="group rounded-xl border border-border bg-surface overflow-hidden hover:shadow-lg hover:border-border-hover transition-all cursor-pointer"
      onClick={onClick}
    >
      {/* Thumbnail */}
      <div className="aspect-[4/3] bg-surface-muted overflow-hidden relative">
        ...
        <button
          ...
          className="absolute top-2 right-2 p-1.5 rounded-lg bg-white/90 dark:bg-neutral-800/90 backdrop-blur-sm shadow-sm text-rose-500 hover:text-rose-600 transition-colors"
        ...
        {item.external_url && (
          <a
            ...
            className="absolute top-2 left-2 p-1.5 rounded-lg bg-white/90 dark:bg-neutral-800/90 backdrop-blur-sm shadow-sm opacity-0 group-hover:opacity-100 transition-opacity"
            title="Otwórz link"
          >
      ...
      <div className="p-3">
        ...
        <div className="mt-2 flex items-center gap-1.5">
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-surface-muted text-text-tertiary truncate max-w-full">
            {item.project_name}
          </span>
        </div>
      </div>
